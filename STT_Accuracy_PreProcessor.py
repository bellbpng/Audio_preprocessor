...
from lib import MS_section_extractor_Beta_Lib
from lib import Voice_Audio_PostProcessor_Beta_Lib
...
def main():###Singing Voice Separation
    ...
    print('done')
    wave_mono=(X[0]+X[1]);
    sf.write('{}_mono.wav'.format(basename), wave_mono.T, sr)
    ...
    print('done')
    sf.write('{}_Instrument.wav'.format(basename), wave.T, sr)
    wave_mono=(wave[0]+wave[1]);
    sf.write('{}_Instrument_mono.wav'.format(basename), wave_mono.T, sr)

    print('inverse stft of vocals...', end=' ')
    v_spec = np.clip(X_mag - pred, 0, np.inf) * X_phase
    wave = spec_utils.spectrogram_to_wave(v_spec, hop_length=args.hop_length)
    print('done')
    sf.write('{}_Voice.wav'.format(basename), wave.T, sr)
    wave_mono=(wave[0]+wave[1]);
    sf.write('{}_Voice_mono.wav'.format(basename), wave_mono.T, sr)
    ...
    #####
    # MS Section Extraction
    path = os.getcwd()
    p_num=0;
    in_name = args.input
    for i in range(0,len(in_name),1):
        if(in_name[i]=='\\'):
            p_num=i
    file_name = in_name[p_num+1:len(in_name)-4]
    th_type=0
    print(file_name)
    MS_section_extractor_Beta_Lib.MS_extractor(path,file_name,th_type);
    
    #####
    # Voice Signal Filtering
    Voice_Audio_PostProcessor_Beta_Lib.MS_Section_Signal_Filtering(path,file_name);
